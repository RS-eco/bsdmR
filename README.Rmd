...
rm(cordex_bioclim_bav_tk4tel); gc()

dat <- bind_rows(aves_tk4tel, odonata_tk4tel) %>% dplyr::full_join(climcur) %>% tidyr::drop_na()
rm(aves_tk4tel, odonata_tk4tel); gc()
...
r_dat <- dat %>% rasterFromXYZ()
climdat <- climcur %>% arrange(x,y) %>% rasterFromXYZ()
var_stack <- raster::crop(climdat, r_dat); rm(climdat, r_dat, climcur); gc()
...
save(bart_pred, file="data/bart_pred.rda", compress="xz"); rm(bart_pred); gc()
```
...
par(mfrow = c(1, 2))
plot(BART_P)
plot(BART_P_01); rm(BART_P, BART_P_01); gc()
...
blocks <- spatialBlock(speciesData = dat_spatial2, species = myspecies, 
...
rm(partial_BART_1, partial_BART_2); gc()
```
